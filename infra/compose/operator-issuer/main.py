"""Mock OAuth2 operator-issuer for local development (feature 002 / ADR-0007 Part 4).

Issues short-lived JWTs under the client-credentials grant for OPERATOR principals.
Audience claim is ``collectmind-operator`` (vs ``collectmind-api`` on the tenant issuer).
Signing key is generated fresh on each container start; the public key is exposed at
``/.well-known/jwks.json``. This is a development convenience only; not for production.

Cloud parity: the operator JWKS endpoint sits behind an internal-only ALB with IAM-scoped
access, signed by a key in AWS Secrets Manager. Same JWK format; verified by the same
...
CONFIG_PATH = Path("/app/operator-issuer-config.yaml")
...
app = FastAPI(title="CollectMind Mock Operator Issuer", version="0.1.0")
