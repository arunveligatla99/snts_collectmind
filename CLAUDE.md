...
- **Phase 6 closure (feature 001 done at `990b437`)**: coverage 86.24% across 214 unit + 41 contract + 14 integration tests; `ruff check` + `ruff format --check` + `mypy --strict` clean; all four CI guards green; production-readiness review at [`docs/runbook/feature-001-readiness-review.md`](docs/runbook/feature-001-readiness-review.md) walks every NON-NEGOTIABLE with a named artifact; three Phase-7 follow-ups explicitly listed (ADR-0002 baseline gated to GPU runner; SC-009 rolling-window logic gated to first CI run; T142 PII-strip CI gate excluded from Phase 6 per the original task-set instruction).
- **Two latent Phase-1 bugs fixed in Phase 6**: `observability/dashboard_provisioner.py`'s `declared_metric_names()` now honors the prometheus_client Counter `_total` suffix stripping; `scripts/check_no_todo_fixme.py` now excludes `.venv*` (was only excluding the bare `.venv` path component, so `.venv-test` got scanned).
