...
- The foundation smoke path (Spec SC-008's 10-minute quickstart on a clean machine) runs with `SLM_PROFILE=dev_default`, a fifth `PolicyGeneratorClient` implementation gated to local-only environments and documented in [ADR-0006](../../docs/adr/0006-dev-default-policy-client.md). It does not satisfy Principle XIII's decode-time-grammar requirement; that deviation is bounded by a startup guard (`COLLECTMIND_ENV != "local"` refuses the client) and a CI pipeline guard (planned T126 in Phase 5). Production and CI runs use the `vllm`, `cpu` (llama.cpp), or `stub` profile per ADR-0002 and ADR-0004. The `dev_default` audit-record convention (`slm_repo="dev/default"`, `slm_revision_sha="0"*40`, `slm_runtime="stub"`) is shared with `FingerprintStubClient` so audit consumers can filter both with a single check.
