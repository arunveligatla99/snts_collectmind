...
- [X] T285 Coverage sweep: brought line coverage to **85.36%** via 8 new unit-test files targeting deployer wrappers, tenant_context middleware, tenant_config repo, ratelimit middleware helpers, auth dependencies, feedback scheduler, tenant_config LISTEN consumer. Verified via `pytest --cov=src/collectmind --cov-fail-under=85`. Principle IV (NON-NEGOTIABLE).
- [X] T286 `ruff check && ruff format --check && mypy --strict src/collectmind` all clean. 180 files formatted; 0 lint errors; "Success: no issues found in 88 source files".
- [X] T287 OpenAPI dump diff: `docs/api/openapi.yaml` regenerated from `python -m collectmind.openapi.dump`. `custom-guards` re-asserts byte-identity on every PR. Principle XVI.
- [X] T288 `scripts/check_no_todo_fixme.py` PASS. Principle III.
- [X] T289 `scripts/check_slm_pinning.py` PASS unchanged. Feature 002 does not touch the SLM boundary. Principle XIV.
- [X] T290 PII-strip CI gate: created [`scripts/check_log_pii.py`](../../scripts/check_log_pii.py) (3 sub-checks: redaction positive, non-PII preservation negative, metric-label PII-fragment scan) + [`tests/unit/test_check_log_pii_gate.py`](../../tests/unit/test_check_log_pii_gate.py). Wired into `.github/workflows/ci.yaml`'s `custom-guards` job. Closes feature-001 SC-007 + feature-002 SC-007. Principle V / Principle IX.
- [X] T291 Threat model extended with three new threats: (7) rate-limit bypass via JWT-issuer forgery; (8) break-glass abuse via operator-key compromise; (9) tenant-vehicle ownership-data integrity attack. Each names defending FRs + verifying tests + residual risk. Principle IX.
- [X] T292 Quickstart re-run on warm Compose stack: **3 seconds** wall-clock vs SC-008 600 s budget (~200× headroom). Principle VIII.
